...
These events are highly sensitive to temperature and other environmental
factors, making phenological records invaluable for understanding how plants
respond to climate variability and change. A plant that flowered on May 15th
in 1950 might now flower on May 1st due to warming temperatures - this shift
of about two weeks represents a significant ecological change.

## The PEP725 Database

The [PEP725 Pan European Phenology Database](http://www.pep725.eu/) is one of
the world's largest collections of plant phenological observations
([Templ et al. 2018](https://doi.org/10.1007/s00484-018-1512-8);
[Templ et al. 2026](https://doi.org/10.1111/nph.70869)). It contains:

- **Over 12 million observations** from across Europe
- **Records spanning 250+ years** (some stations have data back to the 1750s)
- **Thousands of monitoring stations** in more than 20 countries
- **Multiple plant species** including crops, wild plants, and trees

The database is maintained by a network of national phenological services and
uses standardized observation protocols, making it ideal for large-scale
analyses of phenological change.
...
data. The package also automatically loads some small auxiliary datasets (like
global temperature anomalies) that are used by certain functions.

## Getting Phenological Data

The **pep725** package offers four ways to obtain data. Understanding which
...
you must register and agree to terms of use. This creates a problem for
...
- Same distributions of flowering dates across seasons
...
For learning the package, download the pre-generated synthetic dataset:
...
| `genus` | **Plant genus** - The genus name (first part of scientific name) | "Triticum" |
| `species` | **Full species name** - Genus + specific epithet | "Triticum aestivum" |
...
```{r subset}
# Filter to wheat data using data.table syntax
...
CHemische Industrie) is a standardized system for describing plant
...
- The **second digit** (0-9) indicates the secondary stage within that principal stage
...
### Commonly Used BBCH Codes in Phenology
...
When analyzing phenology, you typically focus on specific phases. Here's how:
...
- **Trends**: Is coverage increasing or decreasing?
- **Breakpoints**: Sudden changes (often due to methodology changes)
...
how wheat flowering dates have changed over time.

```{r quick-analysis}
# Step 1: Filter to wheat flowering (BBCH 60 = heading/first flowers)
wheat_flowering <- pep[species == "Triticum aestivum" & phase_id == 60]

cat("Wheat flowering observations:", nrow(wheat_flowering), "\n")
cat("Year range:", min(wheat_flowering$year), "-", max(wheat_flowering$year), "\n")
cat("Countries:", length(unique(wheat_flowering$country)), "\n")
...
if (nrow(annual_mean) > 1) {
  plot(annual_mean$year, annual_mean$mean_doy,
  ...
  if (nrow(annual_mean) >= 3) {
    ...
    # Calculate and display the trend
    slope <- coef(trend)[2]
...
- **Always check for outliers**: A flowering date of DOY 300 (late October) for
  wheat is almost certainly an error
...
- **Leap years**: Remember that DOY 60 is February 29 in leap years, March 1
  otherwise
- **Cross-year comparisons**: Be careful comparing autumn phenology (DOY ~300)
  with spring (DOY ~100)
- **Averaging DOY**: Mean DOY is only meaningful within a phase; don't average
  flowering with harvest!

### Species Considerations

- **Check taxonomy**: "Triticum" might include multiple wheat species
...
- `detect_second_events()` - Find potential second flowering events
