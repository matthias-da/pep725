...
if (nrow(apple_clean) > 0) {
  ...
  if ("anomaly_days" %in% names(anomalies) &&
      any(!is.na(anomalies$anomaly_days))) {

    cat("Anomalies calculated for", sum(!is.na(anomalies$anomaly_days)), "year-groups\n")
    cat("Extreme years:", sum(anomalies$is_extreme, na.rm = TRUE), "\n")

    cat("\n=== STEP 5: Analyze Trends by Decade ===\n\n")

    decade_summary <- anomalies[
      !is.na(anomaly_days),
      .(
...
like global temperature anomalies using `regional_box_ts()`:

```{r regional-climate, eval=FALSE}
# Compile regional data (PEP725 only)
...
- **Data Quality & Irregular Events**: How to detect outliers and identify abnormal events.
