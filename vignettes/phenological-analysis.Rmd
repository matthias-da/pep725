...
# For this vignette, we'll focus on apple (Malus domestica)
# This is one of the best-represented species in PEP725
apple <- pep[species == "Malus domestica"]

# Quick overview of our apple data
cat("Apple observations:", nrow(apple), "\n")
cat("Year range:", min(apple$year), "-", max(apple$year), "\n")
cat("Countries:", length(unique(apple$country)), "\n")
cat("Stations:", length(unique(apple$s_id)), "\n")
cat("Phases recorded:", paste(sort(unique(apple$phase_id)), collapse = ", "), "\n")
```

**Why apple?** *Malus domestica* (domestic apple) is one of the best-represented
species in the database with excellent coverage across Europe. It has clear phenological
phases (bud burst, flowering, fruit development) that are routinely monitored.
...
```{r normals-filtered}
# Normals for apple flowering (BBCH 60) only
