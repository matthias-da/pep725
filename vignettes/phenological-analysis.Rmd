...
Just as meteorologists use "climate normals" (30-year averages of temperature
and precipitation), phenologists use **phenological normals** - long-term
averages of when biological events occur.

**Why are normals important?**

1. **Baseline for comparison**: You can't say "flowering was early this year"
   without knowing when it normally occurs
2. **Detecting change**: Comparing normals from different periods (e.g.,
   1961-1990 vs. 1991-2020) reveals long-term shifts
3. **Spatial patterns**: Normals vary by location, revealing geographic patterns
...
The function returns several statistics for each group. Here's what they mean
...
corresponds to about 1.7 days per decade - a commonly observed rate of
phenological advance in response to climate warming.
...
```{r anomalies-methods-explain}
# The function supports two methods:

# 1. ROBUST method (default, recommended)
#    - Uses MEDIAN as the baseline
#    - Uses MAD (median absolute deviation) for standardization
#    - Less affected by outliers
#    anomalies_robust <- pheno_anomaly(pep, species = "Triticum", robust = TRUE)

# 2. CLASSICAL method
#    - Uses MEAN as the baseline
#    - Uses SD (standard deviation) for standardization
...
For large datasets or repeated analyses, you can compute normals once and
reuse them:

```{r anomalies-precomputed, eval = FALSE}
# Step 1: Compute normals once (slow for large data)
baseline <- pheno_normals(
  pep,
  period = 1990:2010,
...
# Step 2: Use normals for anomaly calculations (fast)
anomalies_2015 <- pheno_anomaly(
  pep[year == 2015],
  normals = baseline  # Pass pre-computed normals
)

anomalies_2016 <- pheno_anomaly(
  pep[year == 2016],
  normals = baseline  # Reuse same normals
)
```
...
- **Data Quality & Irregular Events**: How to detect outliers and identify
  potential second flowering events.
