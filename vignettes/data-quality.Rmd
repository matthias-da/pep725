...
Phenological datasets, including PEP725, are collected by many observers across
decades. This introduces several types of issues:

| Issue Type | Examples | Consequence if Ignored |
|------------|----------|------------------------|
| **Recording errors** | Typos (DOY 150 instead of 105), wrong year | Outliers bias statistics |
| **Identification errors** | Similar species confused | Inconsistent time series |
| **Protocol changes** | Definition of "flowering" changed | Artificial trends |
| **Biological anomalies** | Second flowering, climate extremes | May be scientifically interesting! |

**The key insight**: Not all unusual observations are errors. Some represent
genuine biological phenomena that deserve study rather than deletion.
...
| Part 2 | Second flowering and repeated events | `detect_second_events()` |
| Part 3 | Temporal coverage and completeness | `pep_completeness()` |
| Part 4 | Phase presence validation | `check_phases()` |
| Part 5 | Integrated workflow | Combining all approaches |
...
3. **Second flowering** or other repeated phenological events
   - Plants flowering again in autumn after spring flowering
...
# Focus on very late events (potential second flowering)
plot_outliers(outliers, type = "seasonal", late_threshold = 250)
```

---

# Part 2: Detecting Second Flowering Events

## What is Second Flowering?

**Second flowering** (also called "second bloom" or "re-flowering") occurs when
plants flower again after their normal flowering period, typically in autumn
after spring flowering. This phenomenon has several causes:
...
### Causes of Second Flowering

| Cause | Mechanism | Example |
|-------|-----------|---------|
| **Warm autumn** | Accumulated heat breaks dormancy | Indian summer in October |
| **Drought stress release** | Rain after drought triggers flowering | Late summer rains |
| **Climate disruption** | Normal seasonal cues disrupted | Warm September following hot summer |
| **Species-specific** | Some species naturally flower twice | Some Rosaceae species |

### Why Study Second Flowering?

Second flowering is increasingly observed across Europe and may indicate:

1. **Climate change impacts**: Changing seasonal temperature patterns
2. **Ecosystem disruption**: Mismatches between plants and pollinators
3. **Plant stress indicators**: Response to unusual conditions
4. **Agricultural implications**: Impacts on fruit set and resource allocation

## Detection Methods

The `detect_second_events()` function uses two complementary approaches:

### Method 1: Late Season Detection

Identifies observations occurring after the expected flowering window:

```{r second-methods}
# Method 1: Late season detection (events after expected window)
late_events <- detect_second_events(
  pep = flowering,
  phases = c(60, 65),
  method = "late_season",
  late_threshold = 250  # After September 7 (DOY 250)
)

print(late_events)
```

**Understanding the `late_threshold` parameter:**

| DOY | Date | Interpretation |
|-----|------|----------------|
| 220 | August 8 | Strict - catches more events |
| 250 | September 7 | Moderate (recommended) |
| 280 | October 7 | Conservative - only very late events |

### Method 2: Multiple Observations Per Year

Identifies stations that report the same phase twice in one year:

```{r second-multiple, eval=FALSE}
# Method 2: Multiple observations per year at same station
multiple_events <- detect_second_events(
  pep = flowering,
  method = "multiple_per_year",
  gap_threshold = 60  # At least 60 days between observations
)

# Method 3: Both methods combined (most comprehensive)
all_events <- detect_second_events(
  pep = flowering,
  method = "both"
)
```

**The `gap_threshold` parameter:**

This ensures that "multiple events" are truly separate flowering periods, not
just re-observations of the same event. A 60-day gap ensures the plant had time
to finish the first flowering and initiate a new one.
...
## Visualizing Second Events

### Overview of Detected Events

```{r second-plot, fig.height=5}
# Overview of detected events
plot(late_events, type = "overview")
```

### Timeline View

```{r second-timeline, fig.height=5}
# Timeline showing when events occur
plot(late_events, type = "timeline")
```

## Assessing Confidence

Not all detected events are truly second flowering. Use these criteria to
assess confidence:

| Confidence Level | Criteria | Action |
|------------------|----------|--------|
| **High** | Same station reports phase twice with 60+ day gap | Likely real - include in analysis |
| **Medium** | Single late observation clearly outside normal window | Plausible - verify if possible |
| **Low / Uncertain** | Isolated observation, near boundary | Could be error - investigate further |

### Filtering by Confidence

```{r second-filter}
# Focus on high-confidence events (large z-scores indicate very unusual timing)
if (nrow(late_events$events) > 0) {
  high_conf <- late_events$events[abs(z_score) > 5]
  cat("High-confidence events (|z| > 5):", nrow(high_conf), "\n")
  if (nrow(high_conf) > 0) {
    print(high_conf[1:min(5, nrow(high_conf)),
                    .(species, year, day, z_score)])
  }
}
```
...
- Are heading and harvest phases both available for wheat?
- Which phases are missing for my species of interest?

## Checking Phase Presence

```{r check-basic}
# Check if expected phases are present for wheat
wheat <- pep[species == "Triticum aestivum"]

phase_check <- check_phases(
  pep = wheat,
  expected = c(60, 65, 100)  # heading, flowering, harvest
)
...
multi_check <- check_phases_multi(
  pep = pep,
  species_list = c("Triticum aestivum", "Malus domestica"),
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 4: Investigate late-season outliers for second events
# ══════════════════════════════════════════════════════════════════════════════
# Why: Late outliers may be second flowering, not errors

second <- detect_second_events(
  pep_filtered[phase_id %in% c(60, 65)],
  late_threshold = 250
)

cat("Detected", nrow(second$events), "potential second flowering events\n")

# ══════════════════════════════════════════════════════════════════════════════
# STEP 5: Make informed decisions about exclusion
# ══════════════════════════════════════════════════════════════════════════════
...
# Option C: Keep second flowering events for separate analysis
second_events_ids <- second$events$obs_id
pep_main <- outliers[!obs_id %in% second_events_ids & is_outlier == FALSE]
pep_second <- outliers[obs_id %in% second_events_ids]

# ══════════════════════════════════════════════════════════════════════════════
# STEP 6: Proceed with analysis on cleaned data
# ══════════════════════════════════════════════════════════════════════════════
...
3. Outlier disposition:
   ...
   - Retained: W observations identified as potential second flowering
...
2. **Check seasonally**: Late-season outliers (DOY > 250) may be biologically
   meaningful second flowering events rather than errors.
...
## For Second Flowering Detection
...
3. **Look at weather context**: Link unusual events to temperature anomalies -
   warm autumn weather is a common trigger.

4. **Distinguish from errors**: Very isolated observations (single station,
   single year) need verification before treating as second flowering.

5. **Keep for separate analysis**: Second flowering events are scientifically
   interesting - don't just delete them!
...
| `detect_second_events()` | Find potential second flowering | List of candidate events with confidence |
...
2. **Not all outliers are errors** - some represent genuine biological phenomena
   like second flowering that deserve study.
