...
![Package workflow. Data access functions produce a `pep` object that supports class-preserving subsetting. Quality and validation, core analysis, and advanced analysis functions accept `pep` input and return S3 objects with dedicated `print()`, `summary()`, and `plot()` methods. Standalone visualization functions operate directly on the `pep` object; utility functions provide supporting calculations.\label{fig:workflow}](figures/workflow_diagram.png){ width=95% }
